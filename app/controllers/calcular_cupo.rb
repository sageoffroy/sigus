...
#
    ...
    agents_of_service.each do |agent_of_service|
      ...
      if !(agent_of_service.agent.agent_type.nil?)
        if !(agent_of_service.agent.agent_type.description == "Médico Residente")
          observacion = Observation.where(year:year).first  //Ver el mes
    ...
    novedades = Novelties.where(month: month, year: year, service_of_dependence: service_of_dependence, report_type: "PGuardias Activas", order_by(year_ref, month_ref))
